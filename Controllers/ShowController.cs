    ...
    public class ShowController : Controller
    {
        ...
        private List<ShowsViewItem> GetMovies(List<ShowsViewItem> ShowsView)
        {
            foreach (var cinemaItem in ShowsView)
                using (SqlCommand getMoviesPlayed = new SqlCommand(@"Select Movie.[Original_Name], CAST(SUM((UserRating.Cleanliness + UserRating.Popcorn + UserRating.Screen + UserRating.Seat + UserRating.Sound)/5)/COUNT(UserRating.Cleanliness) AS FLOAT) 
            ...
            )
                {
                    using (SqlDataReader movieReader = getMoviesPlayed.ExecuteReader())
                    {
                        while (movieReader.Read())
                        {
                            ...
                            movie.averageRating = (movieReader.IsDBNull(1)) ?  0f : movieReader.GetDouble(1);
                        }
